...
export function normalizeBrazilMobileNineAfter55(digitsWith55) {
  ...
  if (after55.length === 10) {
    ...
    // Oito dígitos começando em 9: às vezes é “9 duplicado” (99xxxxxxxx) colado errado; remove só o primeiro 9.
    if (sub8[0] === "9") {
      const stripped = sub8.replace(/^9/, "");
      if (stripped.length === 8 && /^[6789]/.test(stripped)) return `55${ddd}9${stripped}`;
      return d;
    }
    if (/^[678]/.test(sub8)) return `55${ddd}9${sub8}`;
    return d;
  }
